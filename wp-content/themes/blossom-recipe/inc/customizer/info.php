...
function blossom_recipe_customizer_theme_info( $wp_customize ) {
    ...
    $theme_info = '<p>';
	$theme_info .= sprintf( __( 'Demo Link: %1$sClick here.%2$s', 'blossom-recipe' ),  '<a href="' . esc_url( 'https://blossomthemes.com/theme-demo/?theme=' . BLOSSOM_RECIPE_THEME_TEXTDOMAIN ) . '/&utm_source=blossom-recipe&utm_medium=customizer&utm_campaign=theme_demo" target="_blank">', '</a>' );
    $theme_info .= '</p><p>';
    $theme_info .= sprintf( __( 'Documentation Link: %1$sClick here.%2$s', 'blossom-recipe' ),  '<a href="' . esc_url( 'https://docs.blossomthemes.com/docs/blossom-recipe/' ) . '?utm_source=blossom-recipe&utm_medium=customizer&utm_campaign=docs" target="_blank">', '</a>' );
    $theme_info .= '</p>';
	...
	$manager->add_section(
		new Blossom_Recipe_Customize_Section_Pro(
			...
			array(
				...
				'pro_url'  => esc_url( 'https://blossomthemes.com/wordpress-themes/blossom-recipe-pro/?utm_source=blossom-recipe&utm_medium=customizer&utm_campaign=upgrade_to_pro' ),
			)
		)
	);
}
